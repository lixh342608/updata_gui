...
class tk_Msginput:
    ...
    def colback(self):
        if self.col==0:
            ...
            self.col=dict.fromkeys(("localpath","cal","update_num","backdir"),"")
    ...
    def tk_input(self):
        self.colback()
        #设置文件路径
        lab1=Label(self.root,text="请在下方输入远程主机信息（格式：root@192.168.1.5:22）",font = 'Helvetica -15 bold').grid(row=0,column=0,columnspan=3)
        ...
        b6=Button(self.root,text="退出程序",command=(lambda x=self.root:x.destroy()),bd=3,overrelief=FLAT,fg="blue",width=18,height=2).grid(row=11,column=1)
        check_var=IntVar()
        cbut1=Checkbutton(self.root,text="重置环境配置",variable=check_var,command=set_but).grid(row=11,column=3)
        b7=Button(self.root,text="重置环境配置",command=change_seting,bd=3,width=15,height=2,state=DISABLED,fg="blue")
        b7.grid(row=11,column=4)
        self.root.mainloop()
